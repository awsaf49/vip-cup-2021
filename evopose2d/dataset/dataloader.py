...
def parse_record(record):
    feature_description = {
        ...
        'joints': tf.io.FixedLenFeature([14*3, ], tf.int64),
...
def preprocess(id, img, bbox, kp, score, DATASET, split='train', predict_kp=False, do_transform=True):
    ...
    if split == 'train':
        ...
        if DATASET.COLOR_AUG:  
            img = tf.image.random_hue(img, DATASET.HUE)
            img = tf.image.random_saturation(img, DATASET.SAT[0], DATASET.SAT[1])
            img = tf.image.random_contrast(img, DATASET.CONT[0], DATASET.CONT[1])
            img = tf.image.random_brightness(img, DATASET.BRI)
    else:
        angle = 0.
